...
class FastGRNNCellKeras(RNNCell):
    ...
    def __init__(self, hidden_size, gate_non_linearity="sigmoid",
                 ...
                 zetaInit=1.0, nuInit=-4.0, name="FastGRNN", **kwargs):#, reuse=None):
        ...
        if uRank is not None:
            self._num_weight_matrices[1] += 1
        self._name = name
        super(FastGRNNCellKeras, self).__init__(**kwargs)#_reuse=reuse)
        #self._reuse = reuse
    ...
    def call(self, inputs, state, training=None):
        ...
        if self._uRank is None:
            uComp = K.dot(state, self.U)
        else:
            uComp = K.dot(
                K.dot(state, self.U1), self.U2)

        pre_comp = wComp + uComp

        z = gen_non_linearity_keras(pre_comp + self.bias_gate,
                              self._gate_non_linearity)

        c = gen_non_linearity_keras(
            pre_comp + self.bias_update, self._update_non_linearity)
        new_h = z * state + (K.sigmoid(self.zeta) * (1.0 - z) +
                             K.sigmoid(self.nu)) * c
        return new_h, new_h
